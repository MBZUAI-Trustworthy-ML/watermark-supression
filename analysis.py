...
def analyze_spoofing_success(file_path, num_keys=4, fpr_tau=0.01, ethics_threshold=6.5):
    """Analyze spoofing success rates for both corrected and uncorrected thresholds.
    ...
    Args:
        ...
        num_keys: Number of watermarking keys
        fpr_tau: Desired false positive rate for spoofing detection (e.g., 0.01)
        ethics_threshold: Ethics score threshold (default: 6.5)
        
    Returns:
        dict: Results for both corrected and uncorrected analysis, for each ethics method
    """
    print(f"Analyzing: {file_path}")
    print(f"Keys: {num_keys}, FPR threshold: {fpr_tau}")
    print(f"Ethics threshold: {ethics_threshold}")
    ...
    # Initialize counters for both ethics methods
    total_samples = 0
    
    # Track which ethics methods are available
    ethics_methods = ['GPT_Ethics', 'SelfEthics']
    
    # Initialize counters for each ethics method
    results_by_ethics = {}
    for ethics_method in ethics_methods:
        results_by_ethics[ethics_method] = {
            'samples_with_score': 0,
            'harmful_samples': 0,  # ethics < threshold
            'harmless_samples': 0,  # ethics >= threshold
            'alg1_success': [0, 0],  # [uncorrected, corrected] - detection + harmful
            'alg2_success': [0, 0],  # [uncorrected, corrected] - detection + harmful
            'alg1_detection_only': [0, 0],  # detection but not harmful
            'alg2_detection_only': [0, 0],  # detection but not harmful
            'ethics_scores': []
        }
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            gpt_ethics = data.get('GPT_Ethics')
            self_ethics = data.get('SelfEthics')
            
            # Process each available ethics method
            available_ethics = []
            if gpt_ethics is not None:
                available_ethics.append(('GPT_Ethics', gpt_ethics))
            if self_ethics is not None:
                available_ethics.append(('SelfEthics', self_ethics))
            ...
            alg1_detected = [highest_z >= uncorrected_tau, highest_z >= corrected_tau]
            alg2_detected = [uncorrected_count == 1, corrected_count == 1]
            
            # Process results for each available ethics method
            for ethics_name, ethics_score in available_ethics:
                results = results_by_ethics[ethics_name]
                ...
                if is_harmful:
                    ...
                    results['harmless_samples'] += 1
                
                # Algorithm 1: Count successes (detection + harmful)
                for i in range(2):  # uncorrected, corrected
                    if alg1_detected[i]:
                        if is_harmful:
                            results['alg1_success'][i] += 1
                        else:
                            results['alg1_detection_only'][i] += 1
                
                # Algorithm 2: Count successes (detection + harmful)
                for i in range(2):  # uncorrected, corrected
                    if alg2_detected[i]:
                        if is_harmful:
                            results['alg2_success'][i] += 1
                        else:
                            results['alg2_detection_only'][i] += 1
    
    # Calculate final results
    final_results = {}
    
    for ethics_method in ethics_methods:
        data = results_by_ethics[ethics_method]
        
        # Skip if no samples for this ethics method
        if data['samples_with_score'] == 0:
            continue
            
        samples_count = data['samples_with_score']
        
        final_results[ethics_method] = {
            'config': {
                'total_samples': total_samples,
                'samples_with_score': samples_count,
                'harmful_samples': data['harmful_samples'],
                'harmless_samples': data['harmless_samples'],
                'num_keys': num_keys,
                'fpr_tau': fpr_tau,
                'ethics_threshold': ethics_threshold,
                'uncorrected_threshold': uncorrected_tau,
            ...
            'uncorrected': {
                'algorithm1': data['alg1_success'][0] / samples_count,
                'algorithm2': data['alg2_success'][0] / samples_count,
                'algorithm1_detection_only': data['alg1_detection_only'][0] / samples_count,
                'algorithm2_detection_only': data['alg2_detection_only'][0] / samples_count
            },
            'corrected': {
                'algorithm1': data['alg1_success'][1] / samples_count,
                'algorithm2': data['alg2_success'][1] / samples_count,
                'algorithm1_detection_only': data['alg1_detection_only'][1] / samples_count,
                'algorithm2_detection_only': data['alg2_detection_only'][1] / samples_count
            },
            'ethics': {
                'average': np.mean(data['ethics_scores']),
                'min': min(data['ethics_scores']),
                'max': max(data['ethics_scores']),
                'harmful_rate': data['harmful_samples'] / samples_count
            }
        }
...
def print_results(results):
    """Print analysis results in a clean format."""
    print(f"\n{'='*80}")
    print("SPOOFING SUCCESS RATES (Detection + Harmful Content)")
    print(f"{'='*80}")
    
    for ethics_method, data in results.items():
        config = data['config']
        
        print(f"\n--- {ethics_method} Results ---")
        print(f"Samples analyzed: {config['samples_with_score']}")
        print(f"Harmful samples (ethics < {config['ethics_threshold']}): {config['harmful_samples']} ({data['ethics']['harmful_rate']:.1%})")
        print(f"Harmless samples: {config['harmless_samples']}")
        
        eth = data['ethics']
        print(f"Ethics avg: {eth['average']:.2f} (range: {eth['min']:.2f}-{eth['max']:.2f})")
        
        print(f"\nThresholds - FPR: {config['fpr_tau']}")
        print(f"  Uncorrected: {config['uncorrected_threshold']:.3f}")
        print(f"  Sidak-corrected: {config['corrected_threshold']:.3f}")
        ...
        for name, key in algorithms:
            uncorr = data['uncorrected'][key]
            corr = data['corrected'][key]
            diff = uncorr - corr
            print(f"{name:<25} {uncorr:<12.4f} {corr:<12.4f} {diff:<12.4f}")
        
        # Print detection-only results (for reference)
        # print(f"\nDetection Only (including harmless content):")
        # for name, key in algorithms:
        #     uncorr = data['uncorrected'][f'{key}_detection_only']
        #     corr = data['corrected'][f'{key}_detection_only']
        #     print(f"{name:<25} {uncorr:<12.4f} {corr:<12.4f}")

def analyze_file(file_path, num_keys=4, fpr_tau=0.01, ethics_threshold=6.5):
    """Main analysis function.
    ...
    Args:
        ...
        num_keys: Number of watermarking keys used
        fpr_tau: Desired false positive rate for spoofing detection (e.g., 0.01, 0.001, 0.00003)
        ethics_threshold: Ethics score cutoff (default: 6.5)
        
    Returns:
        dict: Analysis results for both corrected and uncorrected thresholds, for each ethics method
    """
    # Run analysis
    results = analyze_spoofing_success(
        file_path=file_path,
...
def calculate_averages(file_path):
    """
    Simple function to calculate averages for PPL, GPT, GPT_Ethics, SelfEthics
    ...
    for metric, values in metrics.items():
        if values:
            ...
            print(f"{metric:<12}: {avg:.4f} (n={len(values)})")
        else:
            print(f"{metric:<12}: No valid data")
...
# Usage
if __name__ == "__main__":
    file_path = input("Enter your results file path: ").strip() or "results.txt"
